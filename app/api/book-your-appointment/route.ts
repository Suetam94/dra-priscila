...
import { bookClinicSchema, type BookClinicData } from '@/app/lib/schemas/book-appointment'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = bookClinicSchema.safeParse(body)
    if (!parsed.success) return NextResponse.json({ ok: false, error: parsed.error.message }, { status: 400 })
    const db = getFirestore()
    const ref = await db.collection(collectionName).add(parsed.data as BookClinicData)
