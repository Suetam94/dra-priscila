...
import { doctorInfoSchema, type DoctorInfoData } from '@/app/lib/schemas/doctor-info'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = doctorInfoSchema.safeParse(body)
    if (!parsed.success) return NextResponse.json({ ok: false, error: parsed.error.message }, { status: 400 })
    const db = getFirestore()
    const snap = await db.collection(collectionName).limit(1).get()
    const payload: DoctorInfoData & { updatedAt: string; updatedBy: string } = {
      ...parsed.data,
