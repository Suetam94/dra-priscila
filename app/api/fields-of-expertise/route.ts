...
import { expertiseSchema, type ExpertiseData } from '@/app/lib/schemas/fields-of-expertise'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = expertiseSchema.safeParse(body)
    if (!parsed.success) return NextResponse.json({ ok: false, error: parsed.error.message }, { status: 400 })
    const db = getFirestore()
    const ref = await db.collection(collectionName).add(parsed.data as ExpertiseData)
