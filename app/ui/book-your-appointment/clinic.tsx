...
const Clinic = ({ name, image, contact, address }: IClinicProps) => {
  ...
  return (
    <div key={name} className="flex flex-col md:flex-row items-center mb-5 bg-white rounded-lg shadow-lg p-4 h-[350px]">
      ...
      <div className="w-full md:w-1/2 p-4 text-center md:text-left flex flex-col justify-start">
        <h3 className="text-2xl font-bold text-base-blue mb-4">{name}</h3>
        <div className="flex items-center text-base-blue text-lg font-medium mb-2">
          <MapPin className="w-6 h-6 text-base-blue mr-2" />
          <span>{address}</span>
        </div>
        {contact.phones.map((phone) => (
          <div key={phone.number} className="text-base-blue text-lg font-medium mb-2 flex items-center">
            {phone.isWhatsapp
              ? (
              <a href={getWhatsappLink(phone.ddd, phone.number)} target="_blank" rel="noopener noreferrer" className="flex items-center">
                <WhatsappLogo className="w-6 h-6 text-base-pink mr-2" />
                <span className="text-base-pink">{formatPhoneNumber(phone.ddd, phone.number)}</span>
              </a>
                )
              : (
              <div className="flex items-center">
                <Phone className="w-6 h-6 text-base-blue mr-2" />
